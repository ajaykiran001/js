...
function addTask() {
    ...
    if (task.value != "" && description.value != "") {
        ...
        let taskObject = {
            'slno': tdSno.innerHTML,
            'date':tdDate.innerHTML,
            'task':tdTask.innerHTML,
            'description':tdDescription.innerHTML,
            'priority':tdPriority.innerHTML,
            'completionDate':tdCompletionDate.innerHTML,
            'status':tdStatus.innerHTML,
        
        }
       

            let tasaa= JSON.parse(localStorage.getItem('task'))||[]
            console.log(tasaa);
            tasaa.push(taskObject)
// console.log(taskArray);
 localStorage.setItem('task', JSON.stringify(tasaa))
